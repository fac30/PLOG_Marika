...
- I implemented global state management with React Context to manage the shopping cart state across components (`ProductCard`, `ShopCart`, `Header`). Using `useReducer`, I centralised cart actions, including adding, removing, and updating quantities.
...
- I created a custom hook, `useCartContext`, to simplify accessing cart state and actions across components. 
...
- I refactored several components, and it was a long job, to ensure TypeScript type safety. Below an example of how I defined interfaces for props and state to eliminate implicit `any` types, reducing potential runtime errors.
